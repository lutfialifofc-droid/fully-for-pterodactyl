...
show_menu() {
    ...
    echo -e "${YELLOW}═══════════════════════════════════════════════════════════${NC}"
    echo ""
    echo -e "${PURPLE}=== PANEL ===                                                       === WINGS (NODE) ===${NC}"
    echo -e "${CYAN}[1]${NC} 🚀 Install Pterodactyl Panel                             [4] 🦅 Install Wings (Daemon/Node)"
    echo -e "${CYAN}[2]${NC} ⚡ Optimize Panel                                        [5] 🔧 Manage Wings (Start/Stop/Restart)"                                         
    echo -e "${CYAN}[3]${NC} 🔄 Update Panel                                          [6] 📝 Configure Wings"
    echo ""
    echo -e "${PURPLE}=== MONITORING & MAINTENANCE ===                                    ===  OTHER ===${NC}"
    echo -e "${CYAN}[7]${NC} 📊 Check System Status                                   [10] 📖 View Documentation"
    echo -e "${CYAN}[8]${NC} 💾 Backup Panel & Database                               [11] ❌ Exit"
    echo -e "${CYAN}[9]${NC} 🔧 Troubleshooting & Repair"                                {SCRIPT INI GRATIS!!!}                                                                
    echo ""
    echo -e "${YELLOW}═══════════════════════════════════════════════════════════${NC}"
